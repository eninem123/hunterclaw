...
### 10. 猎手盘中监测推送（仅交易时段盘中关键时间点）
检查当前时间：
- 仅在 **工作日 9:30-15:00** 内执行
- 休市、周末、节假日 → 跳过，不推送
检查 `/root/.openclaw/workspace/pending-summaries/` 目录：
- `stock-monitor-*.md` — 盘中监测快照文件
- 由cron在关键时间点（9:15/9:25/9:35/10:30/13:00/14:00/14:50）生成
- 格式：「📊 【猎手盘中监测 HH:MM】」
- 发送后保留文件（用于盘中连续追踪）
- ⚠️ 只在探测到关键价格事件时推送（突破入场/风控/止损位），无事件静默
